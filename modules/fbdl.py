...
async def _(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        kk = f"{filename}.mp4"
        caption = f"Facebook Video downloaded Successfully by @RaVaN_manager_Bot.\nSay hi to devs @itznik_x."

        await tbot.send_file(
            event.chat_id,
            kk,
            caption="Facebook Video downloaded Successfully by @RaVaN_manager_Bot.\nSay hi to devs @itznik_x.",
        )
        os.system(f"rm {kk}")
    else:
