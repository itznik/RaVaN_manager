...
async def _(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "RaVaN found some torrents for you. Take a look 👇",
        buttons=[
            [
                Button.inline(
                    "📤 Get Torrents from nik's API",
...
async def paginate_prevtorrent(event):
    ...
    try:
        ...
        await event.reply("Sorry, RaVaN Cant found any torrents for that word")
        print(e)
        return
    vector = len(results)
    if num < 0:
...
async def torrentstop(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "Thanks for using.\n❤️ from [RaVaN](t.me/RaVaN_manager_Bot) !",
        link_preview=False,
    )
...
Special Credits to nik for api and also for julia project
"""
