...
from RaVaN.db.mongo_helpers.lockurl import add_chat, get_session, remove_chat
from RaVaN.function.pluginhelpers import (
    admins_only,
    edit_or_reply,
    get_url,
    member_permissions,
...
async def hi(client, message):
    ...
    if not "can_delete_messages" in (await member_permissions(message.chat.id, BOT_ID)):
        sedlyf = await message.reply_text(
            "**I don't have enough permissions to delete messages here(╥﹏╥)**"
        )
