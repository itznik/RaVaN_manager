...
from RaVaN.function.telethonbasics import is_admin
from RaVaN.services.events import register
from RaVaN.services.mongo import mongodb as db
from RaVaN.services.sql.nsfw_watch_sql import (
    add_nsfwatch,
    get_all_nsfw_enabled_chat,
    is_nsfwatch_indb,
    rmnsfwatch,
)
from RaVaN.services.telethon import tbot
...
async def ws(event):
    ...
    if hmmstark is True:
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n[{st}](tg://user?id={hh}) your message contain NSFW content.. So, RaVaN deleted the message\n\n **Nsfw Sender - User / Bot :** [{st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By RaVaNAI` \n**#GROUP_GUARDIAN** "
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, RaVaN deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`⚔️Automatic Detections Powered By RaVaNAI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
...
__help__ = """
<b> Group Guardian: </b>
✪ RaVaN can protect your group from NSFW senders, Slag word users and also can force members to use English
